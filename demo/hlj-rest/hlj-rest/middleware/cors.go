...
func (mw *CorsMiddleware) MiddlewareFunc(handler rest.HandlerFunc) rest.HandlerFunc {
	...
	return func(writer rest.ResponseWriter, request *rest.Request) {
		...
		if !corsInfo.IsCors {
			if mw.RejectNonCorsRequests {
				rest.Error(writer, http.StatusForbidden, "Non CORS request", http.StatusForbidden)
				return
			}
		...
		if mw.OriginValidator(corsInfo.Origin, request) == false {
			rest.Error(writer, http.StatusForbidden, "Invalid Origin", http.StatusForbidden)
			return
		}
		...
		if corsInfo.IsPreflight {
			...
			if mw.allowedMethods[corsInfo.AccessControlRequestMethod] == false {
				rest.Error(writer, http.StatusForbidden, "Invalid Preflight Request", http.StatusForbidden)
				return
			}
