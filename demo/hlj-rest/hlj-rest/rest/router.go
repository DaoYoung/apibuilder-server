...
func MakeRouter(routes ...*Route) (*Router, error) {
	...
	r.handlerFunc = r.makeHandlerFunc()
	return r, nil
}

func (rt *Router) Use(middlewares... Middleware) *Router {
	rt.handlerFunc = rt.handlerFunc.Use(middlewares...)
	return rt
}

// Handle the REST routing and run the user code.
func (rt *Router) makeHandlerFunc() HandlerFunc {
	return func(writer ResponseWriter, request *Request) {
		...
		if route == nil {
			...
			if pathMatched {
				...
				Error(writer, http.StatusMethodNotAllowed, "Method not allowed", http.StatusMethodNotAllowed)
				return
			}
...
func (rt *Router) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	adapterHandlerFunc(rt.handlerFunc)(w, r)
}
